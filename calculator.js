...
function updateBaseValues() {
  baseValues.income =
    parseFloat(document.getElementById("income").textContent) || 0;
  baseValues.discount =
    parseFloat(
      document.getElementById("discount-percentage-btc").textContent
    ) || 0;
  baseValues.discountGmt =
    parseFloat(
      document.getElementById("discount-percentage-gmt").textContent
    ) || 0;
  baseValues.profit =
    parseFloat(document.getElementById("profit").textContent) || 0;
  baseValues.profitDollar =
    parseFloat(document.getElementById("profit-dollar").textContent) || 0;
}
...
function recalculateRewards() {
  ...
  const electricityConsomationFinal =
    electricityConsomationBtc * (1 - discount / 100);
  const electricityConsomationFinalGmt =
    electricityConsomationGmt * (1 - discount / 100);
  ...
  document.getElementById("discount-percentage-gmt").textContent = (
    (electricityConsomationFinalGmt + serviceFinalGmt) *
    multiply
  ).toFixed(2);
  document.getElementById("profit").textContent = (
    totalReward * multiply
  ).toFixed(8);
  document.getElementById("profit-dollar").textContent = (
    totalRewardDollar * multiply
  ).toFixed(2);

  if (isGmtMode) {
    document.getElementById("daily-charging-cost").textContent = (
      electricityConsomationFinalGmt + serviceFinalGmt
    ).toFixed(2);
...
document
  ...
  .addEventListener("change", recalculateRewards);
